...
+ (NSString *)screenSize
{
    return [NSString stringWithFormat:@"%.1lf\"", [SSDeviceLibrary screenSizeWithDevice:self.deviceModel]];
}
...
{
    return [NSString stringWithFormat:@"%.1lf%%", self.memoryUsage*100];
}

- (void)startMemoryUsageStringBlockWithTimeInterval:(NSTimeInterval)timeInterval
                             MemoryUsageStringBlock:(UsageStringBlock)MemoryUsageStringBlock
{
    static SSTimer *timer;
    ...
    timer = [SSTimer scheduledTimerWithTimeInterval:timeInterval target:self repeats:YES block:^(NSTimer * _Nonnull timer) {
        if (MemoryUsageStringBlock)
            MemoryUsageStringBlock(self.class.memoryUsageString);
    }];
}
...
{
    return [NSString stringWithFormat:@"%.1lf%%", self.diskUsage*100];
}
